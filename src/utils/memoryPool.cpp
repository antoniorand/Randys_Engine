...
#define SET_BIT_REVERSED  (var,pos) (((uint8_t)var) | (0x40 >> pos))  
...
void RandysEngine::Pool::Bucket::set_blocks_in_use(std::size_t index,std::size_t n) noexcept{
    ...
    for(std::size_t j = 0; j < n; j++){
        ...
        std::byte& byteToRetrieve = m_ledger.operator[]((n+j)/8);
        //Afterwards we have to set the specific bit of the index block
        byteToRetrieve = (std::byte)(((uint8_t)byteToRetrieve) | (128 >> (n+j)%8));
    }
